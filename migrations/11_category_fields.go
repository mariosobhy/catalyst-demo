...
// Adds a direct relation field `category` to tickets, tasks, comments, links, files, timeline, users, and features so users can pick a category
...
func categoryFieldsUp(db dbx.Builder) error {
	...
	if err := addField(CommentCollectionName); err != nil {
		return err
	}
	if err := addField(LinkCollectionName); err != nil {
		return err
	}
	if err := addField(fileCollectionName); err != nil { // files
		return err
	}
	if err := addField(TimelineCollectionName); err != nil {
		return err
	}
	if err := addField(UserCollectionName); err != nil {
		return err
	}
	if err := addField(FeatureCollectionName); err != nil {
		return err
	}
...
func categoryFieldsDown(db dbx.Builder) error {
	...
	if err := removeField(CommentCollectionName); err != nil {
		return err
	}
	if err := removeField(LinkCollectionName); err != nil {
		return err
	}
	if err := removeField(fileCollectionName); err != nil { // files
		return err
	}
	if err := removeField(TimelineCollectionName); err != nil {
		return err
	}
	if err := removeField(UserCollectionName); err != nil {
		return err
	}
	if err := removeField(FeatureCollectionName); err != nil {
		return err
	}
	return nil
}
